...
internal class Matrix
{
    ...
    public Matrix(RationalNumber[][] matrixElements)
    {
        _array = new RationalNumber[matrixElements.Length, matrixElements[0].Length];

        for (var row = 0; row <= LastRowIndex; row++)
        {
            for (var column = 0; column <= LastColumnIndex; column++)
            {
                _array[row, column] = matrixElements[row][column];
            }
        }
    }
